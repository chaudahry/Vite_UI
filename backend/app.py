...
def upload_resumes():
    ...
    for file in files:
        ...
        try:
            ...
            # Clean up locally saved file if Appwrite upload fails
...
def screen_resumes():
    ...
    try:
        ...
        results = []
        screening_results_db.clear() # Clear previous screening results cache
        ...
        for resume_id in resume_ids:
            ...
            screening_result_data = {
                ...
                'filepath': resume_data_doc['filepath'], # Appwrite file URL
                'appwrite_file_id': resume_data_doc['appwrite_file_id'], # Appwrite file ID
            ...
            # Add to in-memory cache for dashboard display
...
def get_dashboard_data():
    # For dashboard, we'll fetch from Appwrite directly to ensure fresh data
    # and handle pagination/filtering if needed in the future.
    # For now, we'll fetch all results for the last job screened.
    # A more robust solution would pass a job_id to this endpoint.

    # If screening_results_db is empty, it means no screening has been done in this session
    # or the session was cleared. We need a way to retrieve past screening results.
    # For simplicity, let's assume the frontend will trigger screening first.
    # If you need to load previous results, you'd query Appwrite for documents
    # in APPWRITE_COLLECTION_SCREENING_RESULTS_ID, possibly filtered by user_id or job_id.

    # For now, we'll rely on the in-memory cache populated by screen_resumes.
    # If you want to persist dashboard data across server restarts, you'd need
    # to query Appwrite here, potentially filtering by a 'current_job_id' or similar.
    ...
    for res in results:
        formatted_results.append({
            'id': res['resume_id'], # This is the document ID of the resume in Appwrite
            ...
            'shortlisted': False # This state is managed by frontend
        })
...
def download_resume_file():
    data = request.json
    # The frontend should ideally send the Appwrite file ID or the resume_id
    # For compatibility, let's assume 'filepath' now contains the Appwrite file URL
    # or we need to fetch the resume document first.
    # Let's adjust to expect 'resume_id' from frontend for direct lookup.
...
def clear_session_data():
    print("Backend in-memory session data cleared.")
